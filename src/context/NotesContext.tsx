...
export function NotesProvider({ children }: { children: ReactNode }) {
        ...
        if (updated.id !== savingNoteId) {
          ...
          if (pinnedIds.includes(savingNoteId)) {
            const updatedSettings = {
              ...currentSettings,
              pinnedNoteIds: pinnedIds.map((id) => (id === savingNoteId ? updated.id : id)),
            };
  ...
  useEffect(() => {
    async function init() {
      try {
        let folder = await notesService.getNotesFolder();

        // On mobile, if no folder is set, use the default notes folder
        if (!folder) {
          const defaultFolder = await notesService.getDefaultNotesFolder();
          if (defaultFolder) {
            await notesService.setNotesFolder(defaultFolder);
        ...
        if (folder) {
          const notesList = await notesService.listNotes();
          setNotes(notesList);
          // Start file watcher (desktop only, noop on mobile)
          await notesService.startFileWatcher();
        }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    listen<{ changed_ids: string[] }>("file-change", (event) => {
      ...
      const externalChanges = changedIds.filter((id) => !recentlySavedRef.current.has(id));
