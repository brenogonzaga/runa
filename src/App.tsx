...
function AppContent() {
  ...
  const [mobileNoteClick, setMobileNoteClick] = useState(0);
  ...
  // On mobile: switch to editor when a note is selected and we're on the list view
  useEffect(() => {
    // Ignore if user manually went back
    if (manualBackRef.current) {
      manualBackRef.current = false;
      return;
    }
    if (selectedNoteId && mobileView === "list") {
      setMobileView("editor");
    }
  }, [selectedNoteId, mobileView, mobileNoteClick]);
  ...
  const handleAiEdit = useCallback(
    async (prompt: string) => {
      ...
      try {
        ...
        if (result.success) {
          ...
          toast(<AiResponseToast output={result.output} provider={aiProvider} />, {
            duration: Infinity,
            closeButton: true,
          ...
          toast.error(
            <div className="space-y-1">
              <div className="font-medium">{t("toasts.aiEditFailed")}</div>
    ...
    [aiProvider, currentNote, reloadCurrentNote, t],
  );
  ...
  if (isLoading) {
    return (
      <div className="h-dvh flex items-center justify-center bg-bg-secondary">
    ...
    <>
      <div className="h-dvh flex bg-bg overflow-hidden">
        {view === "settings" ? (
          ...
          <>
            {/* Notes panel: full-screen on mobile, fixed-width sidebar on desktop */}
            <div
              className={cn(
                "overflow-hidden flex flex-col h-full max-md:w-full",
                mobileView === "editor" && "max-md:hidden",
                "md:transition-all md:duration-500 md:ease-out",
                !sidebarVisible || focusMode
                  ? "md:opacity-0 md:-translate-x-4 md:w-0 md:pointer-events-none"
                  : "md:opacity-100 md:translate-x-0 md:w-64",
              )}
            >
              <Sidebar
...
function UpdateToast({ update, toastId }: { update: Update; toastId: string | number }) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      toast.success(t("toasts.updateInstalled"), {
        duration: Infinity,
        closeButton: true,
      });
    } catch (err) {
  ...
  return (
    <div className="flex flex-col gap-1">
      <div className="font-medium text-sm">
        {t("dialogs.updateAvailable", { version: update.version })}
      </div>
      {update.body && <div className="text-xs text-text-muted line-clamp-3">{update.body}</div>}
      <button
