...
export function ToolbarButton({
  ...
  const button = (
    <button
      className={cn(
        "h-10 w-10 md:h-7 md:w-7 flex items-center justify-center text-sm rounded transition-colors shrink-0",
        "focus:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-1",
        isActive ? "bg-bg-muted text-text" : "hover:bg-bg-muted text-text-muted",
        className,
      )}
      tabIndex={-1}
      aria-label={title}
...
const iconButtonVariants = {
  ...
  outline: "border border-border text-text-muted hover:bg-bg-muted hover:text-text",
};

export const IconButton = React.forwardRef<HTMLButtonElement, IconButtonProps>(
  ({ className, children, title, size = "sm", variant = "ghost", ...props }, ref) => {
    const button = (
      <button
        ref={ref}
        className={cn(
...
export function ListItem({
  ...
  return (
    <div
      ...
      tabIndex={-1}
      className={cn(
        "w-full text-left px-2.5 py-3 md:py-2.25 transition-colors cursor-pointer select-none rounded-md",
    ...
    >
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-1 min-w-0">
          ...
          <span className={cn("text-sm font-medium truncate text-text")}>{title}</span>
        </div>
      </div>
    ...
    >
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div className="flex flex-col min-w-0">
          ...
          {subtitle && <span className="text-sm truncate text-text-muted">{subtitle}</span>}
        </div>
      </div>
      {shortcut && (
        <kbd
