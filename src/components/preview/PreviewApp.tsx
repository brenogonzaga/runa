...
export function PreviewApp({ filePath }: PreviewAppProps) {
  ...
  useEffect(() => {
    filesService
      ...
      .catch((error) => {
        ...
        toast.error(`${t("toasts.loadFileFailed")}: ${error}`);
      });
  }, [filePath]);
  ...
  const save = useCallback(
    async (newContent: string) => {
      try {
        ...
        toast.error(`${t("toasts.saveFailed2")}: ${error}`);
      }
    },
    [filePath],
  );
  ...
  const reload = useCallback(async () => {
    try {
      ...
      toast.error(`${t("toasts.reloadFailed")}: ${error}`);
    }
  }, [filePath]);
  ...
  const handleSaveToFolder = useCallback(async () => {
    ...
    try {
      ...
      toast.error(`${t("toasts.saveToFolderFailed")}: ${error}`);
    } finally {
