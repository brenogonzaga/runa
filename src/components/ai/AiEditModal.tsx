...
export function AiEditModal({
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-start md:items-center justify-center pt-4 md:py-11 px-3 md:px-4 pointer-events-none"
          ...
          <div className="flex items-center gap-3 px-4.5 py-3.5">
            ...
            <input
              ...
              placeholder={
                cliInstalled === false
                  ? isCodex
                    ? t("ai.codexNotInstalled")
                    : t("ai.claudeNotInstalled")
                  : t("placeholders.aiPrompt")
              }
        ...
        <div className="p-4.5 space-y-3">
          {isExecuting ? (
            <div className="flex items-center gap-2 text-sm text-text-muted">
              ...
              <span>{isCodex ? t("ai.codexEditing") : t("ai.claudeEditing")}</span>
            </div>
          ) : cliInstalled === false ? (
            <>
              <div className="text-sm space-y-0.5 p-3 bg-orange-500/10 rounded-md ">
                ...
                <div className="text-orange-700/80 dark:text-orange-400/80">
                  ...
                  to install it, then restart Runa.
                </div>
              </div>
            ...
            <div className="flex items-center gap-2 text-sm text-text-muted">
              <SpinnerIcon className="w-4 h-4 animate-spin" />
              <span>{t("ai.checkingFor", { provider: cliName })}</span>
            </div>
          ) : (
            <>
              <div className="text-sm space-y-1 p-3 bg-bg-muted rounded-md">
                <span className="font-medium text-text">{t("ai.howDoesItWork")}</span>{" "}
                <span className="text-text-muted">
                  {t("ai.aiDescription", { provider: providerName, tool: cliName })}
                </span>
              </div>
              <div className="w-full flex flex-wrap gap-3 sm:justify-between">
                <div className="flex items-center gap-1.5 text-sm text-text-muted">
                  <kbd className="text-xs px-1.5 py-0.5 rounded-md bg-bg-muted text-text-muted">
                    Esc
                  </kbd>
