  ...
  return (
    <>
      ...
      <p className="text-xs text-center mt-2 text-text-muted">
        {t("editor.tableSize", { rows: hovered.row, cols: hovered.col })}
      </p>
    </>
  );
}
...
// Separator component - only visible on larger screens
function ToolbarSeparator() {
  return <div className="w-px h-4.5 border-l border-border mx-0.5 shrink-0 hidden md:block" />;
}
...
function FormatBar({ editor, onAddLink, onAddImage }: FormatBarProps) {
  ...
  return (
    <div className="px-2 pb-2 border-b border-border">
      {/* Responsive flex-wrap layout - buttons wrap naturally and center on screen */}
      <div className="flex flex-wrap items-center justify-center gap-0.5">
        {/* Text formatting */}
        <ToolbarButton
          onClick={() => editor.chain().focus().toggleBold().run()}
          isActive={editor.isActive("bold")}
          title={`${t("tooltips.bold")} (${mod}${isMac ? "" : "+"}B)`}
        >
        ...
        <ToolbarButton
          ...
          title={`${t("tooltips.italic")} (${mod}${isMac ? "" : "+"}I)`}
        >
        ...
        <ToolbarButton
          ...
          title={`${t("tooltips.strikethrough")} (${mod}${isMac ? "" : "+"}${shift}${isMac ? "" : "+"}S)`}
        >
        ...
        <ToolbarButton
          ...
          title={`${t("tooltips.heading1")} (${mod}${isMac ? "" : "+"}${alt}${isMac ? "" : "+"}1)`}
        >
        ...
        <ToolbarButton
          ...
          title={`${t("tooltips.heading2")} (${mod}${isMac ? "" : "+"}${alt}${isMac ? "" : "+"}2)`}
        >
        ...
        <ToolbarButton
          ...
          title={`${t("tooltips.heading3")} (${mod}${isMac ? "" : "+"}${alt}${isMac ? "" : "+"}3)`}
        >
        ...
        <ToolbarButton
          ...
          title={`${t("tooltips.heading4")} (${mod}${isMac ? "" : "+"}${alt}${isMac ? "" : "+"}4)`}
        >
        ...
        <ToolbarButton
          ...
          title={`${t("tooltips.bulletList")} (${mod}${isMac ? "" : "+"}${shift}${isMac ? "" : "+"}8)`}
        >
        ...
        <ToolbarButton
          ...
          title={`${t("tooltips.numberedList")} (${mod}${isMac ? "" : "+"}${shift}${isMac ? "" : "+"}7)`}
        >
        ...
        <ToolbarButton
          ...
          title={`${t("tooltips.blockquote")} (${mod}${isMac ? "" : "+"}${shift}${isMac ? "" : "+"}B)`}
        >
        ...
        <ToolbarButton
          ...
          title={`${t("tooltips.inlineCode")} (${mod}${isMac ? "" : "+"}E)`}
        >
        ...
        <ToolbarButton
          ...
          title={`${t("tooltips.codeBlock")} (${mod}${isMac ? "" : "+"}${alt}${isMac ? "" : "+"}C)`}
        >
        ...
        <ToolbarButton
          ...
          title={`${t("tooltips.addLink")} (${mod}${isMac ? "" : "+"}K)`}
        >
        ...
        <ToolbarButton
          ...
          isActive={false}
          title={t("tooltips.insertWikilink")}
        >
          <BracketsIcon className="w-4.5 h-4.5 stroke-[1.5]" />
        </ToolbarButton>
        <ToolbarButton onClick={onAddImage} isActive={false} title={t("tooltips.addImage")}>
...
export function Editor({
  ...
  const reloadCurrentNote = previewMode ? previewMode.reload : notesCtx!.reloadCurrentNote;
  const reloadVersion = previewMode ? previewMode.reloadVersion : notesCtx!.reloadVersion;
  ...
  const getMarkdown = useCallback((editorInstance: ReturnType<typeof useEditor>) => {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          "prose prose-lg dark:prose-invert max-w-3xl mx-auto focus:outline-none min-h-full px-4 sm:px-6 pt-8 pb-24",
      },
          ...
          if (blob) {
            ...
            reader.onload = async () => {
              ...
              try {
                ...
                toast.error(t("toasts.pasteImageFailed"));
              }
            };
            reader.onerror = () => {
              console.error("Failed to read clipboard image:", reader.error);
              toast.error(t("toasts.clipboardImageFailed"));
            };
  ...
  useEffect(() => {
    ...
    const storage = (editor.storage as any).wikilink as WikilinkStorage | undefined;
      ...
      if (wikilinkEl) {
        ...
        if (noteTitle && currentNotes) {
          ...
          if (note) {
            ...
            toast.info(t("toasts.noteNotExist", { title: noteTitle }));
          }
        }
        return;
      }
      ...
      if (link) {
        ...
        if ((e.metaKey || e.ctrlKey) && link.href) {
          if (isAllowedUrlScheme(link.href)) {
            openUrl(link.href).catch((error) => console.error("Failed to open link:", error));
          } else {
            toast.error(t("toasts.linkSchemeError"));
          }
        }
      }
    };
  ...
  }, [editor, currentNote, getMarkdown, t]);
        ...
        <div className="flex-1 flex items-center justify-center pb-8">
          <div className="text-center text-text-muted select-none">
            ...
            <h1 className="text-2xl text-text font-serif mb-1 tracking-[-0.01em] ">
              {t("emptyState.whatsOnMind")}
            </h1>
            <p className="text-sm">{t("emptyState.pickUp")}</p>
            {createNote && (
              <Button onClick={createNote} variant="secondary" size="md" className="mt-4">
                {t("buttons.newNote")}{" "}
    ...
    <div className="flex-1 flex flex-col bg-bg overflow-hidden" {...swipeHandlers}>
      ...
      >
        ...
        >
                ...
                onClick={async () => {
                  ...
                  try {
                    if (isPinned) {
                      ...
                      toast.success(t("toasts.noteUnpinned"));
                    } else {
                      await pinNote(currentNote.id);
                      toast.success(t("toasts.notePinned"));
                    }
          ...
          {currentNote && (
            <Tooltip content={`${t("editor.findInNote")} (${mod}${isMac ? "" : "+"}F)`}>
              <IconButton className="hidden sm:flex" onClick={openEditorSearch}>
          ...
          {currentNote && (
            ...
            >
              <IconButton className="hidden sm:flex" onClick={toggleSourceMode}>
                {sourceMode ? (
          ...
          {currentNote && (
            <DropdownMenu.Root open={moreMenuOpen} onOpenChange={setMoreMenuOpen}>
              <Tooltip content={t("editor.moreActions")}>
                <DropdownMenu.Trigger asChild>
                  <IconButton className="sm:hidden">
                    <MoreHorizontalIcon className="w-4.5 h-4.5 stroke-[1.6]" />
                  </IconButton>
                </DropdownMenu.Trigger>
              </Tooltip>
              <DropdownMenu.Portal>
                ...
                >
                    ...
                    <DropdownMenu.Item
                      ...
                      onSelect={async () => {
                        ...
                        try {
                          ...
                          console.error("Failed to pin/unpin note:", error);
                          toast.error(
                            isPinned ? t("toasts.unpinFailed") : t("toasts.pinFailed"),
                          );
                        }
                      }}
                    >
                      <PinIcon
                        className={cn("w-4 h-4 stroke-[1.3]", isPinned && "fill-current")}
                      />
                      {isPinned ? t("editor.unpinNote") : t("editor.pinNote")}
                    </DropdownMenu.Item>
                  )}
                  <DropdownMenu.Item
                    className="px-3 py-1.5 text-sm text-text cursor-pointer outline-none hover:bg-bg-muted focus:bg-bg-muted flex items-center gap-2"
                  ...
                  >
                    <SearchIcon className="w-4 h-4 stroke-[1.6]" />
                    {t("editor.findInNote")}
                  </DropdownMenu.Item>
                  <DropdownMenu.Item
                    className="px-3 py-1.5 text-sm text-text cursor-pointer outline-none hover:bg-bg-muted focus:bg-bg-muted flex items-center gap-2"
                  ...
                  >
                    {sourceMode ? (
                      <MarkdownOffIcon className="w-4 h-4 stroke-[1.4]" />
                    ) : (
                      <MarkdownIcon className="w-4 h-4 stroke-[1.4]" />
                    )}
                    {sourceMode ? t("editor.viewFormatted") : t("editor.viewMarkdown")}
                  </DropdownMenu.Item>
                </DropdownMenu.Content>
              </DropdownMenu.Portal>
            </DropdownMenu.Root>
          ...
          <DropdownMenu.Root open={copyMenuOpen} onOpenChange={setCopyMenuOpen}>
            <Tooltip
              content={`${t("tooltips.export")} (${mod}${isMac ? "" : "+"}${shift}${isMac ? "" : "+"}C)`}
            >
              <DropdownMenu.Trigger asChild>
                <IconButton>
      ...
      <div
        className={`${focusMode || sourceMode ? "opacity-0 max-h-0 overflow-hidden pointer-events-none" : "opacity-100 max-h-32"} ${hasTransitioned ? "transition-all duration-1000 delay-500" : ""}`}
      >
        ...
        {sourceMode ? (
          ...
          <div className="h-full">
            <textarea
              ...
              dir={textDirection}
              className="w-full h-full bg-transparent text-text focus:outline-none resize-none px-4 sm:px-6 pt-8 pb-24 mx-auto block"
              style={{
